...
-- Table `DBTMM`.`Parole`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTMM`.`Parole` (
...
-- -----------------------------------------------------
-- Table `DBTMM`.`Descrizioni`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTMM`.`Descrizioni` (
  ...
  INDEX `DE_Parola_fk_idx` (`Parola_fk` ASC, `Descrizione`(500) ASC) VISIBLE,
  CONSTRAINT `DE_Parola_fk`
    FOREIGN KEY (`Parola_fk`)
    REFERENCES `DBTMM`.`Parole` (`Parola_pk`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
CREATE TABLE IF NOT EXISTS `DBTMM`.`Esempi` (
  ...
  INDEX `ES_Parola_fk&Esempio_idx` (`Parola_fk` ASC, `Esempio`(500) ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `DBTMM`.`ScrittureGenerali`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTMM`.`ScrittureGenerali` (
...
-- -----------------------------------------------------
-- Table `DBTMM`.`Traduzioni(P-P)`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTMM`.`Traduzioni(P-P)` (
...
AFTER INSERT ON `DBTMM`.`Parole`
FOR EACH ROW
BEGIN
IF new.Lingua = "IT" THEN
	INSERT IGNORE INTO `DBTMM`.`ScrittureGenerali`(`Parola_fk`, `ScritturaGenerale`) VALUES
...
BEFORE INSERT ON `DBTMM`.`Traduzioni(P-P)`
FOR EACH ROW
BEGIN
